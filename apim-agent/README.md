...
To add a new gateway-specific agent (e.g., Kong), update `internal/agent/registry.go` as follows:  

### Import the Gateway Agent
```go
import (
    kongAgent "github.com/wso2/product-apim-tooling/apim-agents/<gateway-agent>"
)
```
...
func init() {
    ...
    agentReg.RegisterAgent("<gateway-agent-name>", &kongAgent.Agent{})
}
```
...
## Conclusion 
This guide covers setting up, building, and deploying the **Common Agent** in **WSO2 APK**. By following these steps, you can register a new gateway agent, build the Common Agent, and deploy it using Helm.